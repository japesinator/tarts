...
zipAndFold : (a -> a -> a) -> (a -> a -> a) -> Vect (S n) a -> Vect (S n) a -> a
...
initializeCount : Vect n a -> Vect n (a, Nat)
initializeCount []        = []
...
countingByteEq a b = (zipAndFold (addCount bitNXor) (addCount bitAnd) (initializeCount a) (initializeCount b))
...
zipOps : (a,b:Vect n c) -> (f:c -> c -> c) -> allHasCount (zipWith (addCount f) (initializeCount a) (initializeCount b)) 1 = True
zipOps []        []        f = refl
zipOps (x :: xs) (y :: ys) f = rewrite (zipOps xs ys f) in
                               refl

-- This is just the above function for specifically bytes
-- FIXME: this doesn't need to exist

zipBytes : (a,b:Byte) -> (f:Bit -> Bit -> Bit) -> allHasCount (zipWith (addCount f) (initializeCount a) (initializeCount b)) (S Z) = True
zipBytes a b f = rewrite zipOps a b f in
                 refl
...
addCountBasic : (f:c -> c -> c) -> (a,b:(c, Nat)) -> snd ((addCount f) a b) = (snd a + snd b) + 1
...
addEightThings : (a,b,c,d,e,f,g,h:Nat) -> plus (plus (b) (plus (plus (c) (plus (plus (d) (plus (plus (e) (plus (plus (f) (plus (plus (g) (plus (plus (h) (a)) 1)) 1)) 1)) 1)) 1)) 1)) 1 =
                                          plus (plus (a) (plus (b) (plus (c) (plus (d) (plus (e) (plus (f) (plus (g) (plus (h) 0)))))))) (fromInteger 7)
addEightThings a b c d e f g h = rewrite plusZeroRightNeutral h in
                                 ...
                                 rewrite plusCommutative d (S (S (S (S (plus (plus (plus (plus h a) g) f) e))))) in
                                 rewrite plusCommutative (plus (S (S (S (S (plus (plus (plus (plus h a) g) f) e))))) d) 1 in
                                 rewrite plusCommutative c (S (S (S (S (S (plus (plus (plus (plus (plus h a) g) f) e) d)))))) in
                                 rewrite plusCommutative (plus (S (S (S (S (S (plus (plus (plus (plus (plus h a) g) f) e) d)))))) c) 1 in
                                 rewrite plusCommutative b (S (S (S (S (S (S (plus (plus (plus (plus (plus (plus h a) g) f) e) d) c))))))) in
                                 rewrite plusCommutative (plus (S (S (S (S (S (S (plus (plus (plus (plus (plus (plus h a) g) f) e) d) c))))))) b) 1 in
                                 ...
                                 rewrite plusCommutative (plus (plus c (plus d (plus e (plus f (plus g h))))) a) b in
                                 rewrite plusAssociative b (plus c (plus d (plus e (plus f (plus g h))))) a in
                                 rewrite plusCommutative (plus b (plus c (plus d (plus e (plus f (plus g h)))))) a in
                                 rewrite plusCommutative (plus a (plus b (plus c (plus d (plus e (plus f (plus g h))))))) 7 in
                                 refl
...
foldrByteBasic : (a:Vect 8 (Bit, Nat)) -> (f:Bit -> Bit -> Bit) -> snd (vFoldr1 (addCount f) a) = sum (map snd a) + 7
foldrByteBasic [a,b,c,d,e,f,g,h] i = rewrite addCountBasic i b (addCount i c (addCount i d (addCount i e (addCount i f (addCount i g (addCount i h a)))))) in
                                     rewrite addCountBasic i c (addCount i d (addCount i e (addCount i f (addCount i g (addCount i h a))))) in
                                     rewrite addCountBasic i d (addCount i e (addCount i f (addCount i g (addCount i h a)))) in
                                     rewrite addCountBasic i e (addCount i f (addCount i g (addCount i h a))) in
                                     rewrite addCountBasic i f (addCount i g (addCount i h a)) in
                                     rewrite addCountBasic i g (addCount i h a) in
                                     rewrite addCountBasic i h a in
                                     rewrite addEightThings (snd a) (snd b) (snd c) (snd d) (snd e) (snd f) (snd g) (snd h) in
                                     refl
...
foldrHomoByte : (a:Vect 8 (Bit,Nat)) -> (f:Bit -> Bit -> Bit) -> (p:allHasCount a (S Z) = True) -> snd (vFoldr1 (addCount f) a) = 15
...
-- foldrHomoByte [(a,(S Z)),(b,(S Z)),(c,(S Z)),(d,(S Z)),(e,(S Z)),(f,(S Z)),(g,(S Z)),(h,(S Z))] i p = refl
...
--   will be performed.
zipAndFoldBasic : (a,b:Byte) -> (f,g:Bit -> Bit -> Bit) -> snd (vFoldr1 (addCount f) (zipWith (addCount g) (initializeCount a) (initializeCount b))) = 15
zipAndFoldBasic a b f g = rewrite foldrHomoByte (zipWith (addCount g) (initializeCount a) (initializeCount b)) f (zipBytes a b g) in
                          refl

zipAndFoldBasic' : (a,b:Byte) -> (f,g:Bit -> Bit -> Bit) -> snd (zipAndFold (addCount f) (addCount g) (initializeCount a) (initializeCount b)) = 15
zipAndFoldBasic' a b f g = rewrite zipAndFoldBasic a b f g in
                           refl

-- This says equality always takes 15 operations

numericTimeConstancy : (a,b:Byte) -> snd (countingByteEq a b) = 15
numericTimeConstancy a b = rewrite zipAndFoldBasic' a b bitNXor bitAnd in
                           refl

-- And thus equality between any two pairs of bytes takes the same amount of
--   time.

timeConstancy : (a,b,c,d:Byte) -> snd (countingByteEq a b) = snd (countingByteEq c d)
timeConstancy a b c d = rewrite numericTimeConstancy a b in
                        rewrite numericTimeConstancy c d in
                        refl
                        -- QED
